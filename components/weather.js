...
export default function Weather({city_name, color_name}) {
    ...
    useEffect(()=>{
        fetch(`${process.env.NEXT_PUBLIC_OW_API_URL}/weather/?q=${city_name}&APPID=${process.env.NEXT_PUBLIC_OW_API_KEY}`)
        .then(res => res.json())
        .then(result => {
            setData(result);
            console.log(result);
  ...
  <div className={`bg-gradient-to-r from-${color_name}-500 to-${color_name}-300
    ...
    <div className="w-full px-8 absolute top-6">
      <div className="flex justify-between">
        ...
        <div>
        <img src={`${process.env.NEXT_PUBLIC_OW_ICON_URL}/${data.weather[0].icon}.png`} 
